...
describe("Validate Amount and Tax for multiple scenario's", () => {
    ...
    it('TC01- Currency = EUR , Qnty = 1, unitprice = 1 , Tax = 0', () => {
        ...
        cy.get('li').contains(invoiceval[count - 1].Currency).click({ force: true })
        getDescriptioninput().clear()
        getDescriptioninput().type(invoiceval[count - 1].description).should('have.value', invoiceval[count - 1].description)
        getQuantity().clear()
        getQuantity().type(invoiceval[count - 1].Qty).should('have.value', invoiceval[count - 1].Qty)
        getunitprice().clear()
        getunitprice().type(invoiceval[count - 1].unitprice).should('have.value', invoiceval[count - 1].unitprice)
        gettaxpercent().clear()
        gettaxpercent().type(invoiceval[count - 1].tax).should('have.value', invoiceval[count - 1].tax)
    ...
    it('TC07 - Add Two items : Currency = MATIC  , Qnty = 20 and 10 , unitprice = 1.75 and 2.95 , Tax = 1.34 and 2.35', () => {
        ...
        cy.get('li').contains(invoiceval[count - 1].Currency).click({ force: true })
        getDescriptioninput().clear()
        getDescriptioninput().type(invoiceval[count - 1].description_1).should('have.value', invoiceval[count - 1].description_1)
        getQuantity().clear()
        getQuantity().type(invoiceval[count - 1].Qty_1).should('have.value', invoiceval[count - 1].Qty_1)
        getunitprice().clear()
        getunitprice().type(invoiceval[count - 1].unitprice_1).should('have.value', invoiceval[count - 1].unitprice_1)
        gettaxpercent().clear()
        gettaxpercent().type(invoiceval[count - 1].tax_1).should('have.value', invoiceval[count - 1].tax_1)
        getAddItemButton().click({ force: true })
        cy.wait(2000)
        cy.get('input[name="invoiceInclusions.1.description"]').type(invoiceval[count - 1].description_2).should('have.value', invoiceval[count - 1].description_2)
        cy.get('input[name="invoiceInclusions.1.quantity"]').clear()
        cy.get('input[name="invoiceInclusions.1.quantity"]').type(invoiceval[count - 1].Qty_2).should('have.value', invoiceval[count - 1].Qty_2)
        cy.get('input[name="invoiceInclusions.1.unitPrice.value"]').clear()
        cy.get('input[name="invoiceInclusions.1.unitPrice.value"]').type(invoiceval[count - 1].unitprice_2).should('have.value', invoiceval[count - 1].unitprice_2)
        cy.get('input[name="invoiceInclusions.1.taxPercent"]').clear()
        cy.get('input[name="invoiceInclusions.1.taxPercent"]').type(invoiceval[count - 1].tax_2).should('have.value', invoiceval[count - 1].tax_2)
